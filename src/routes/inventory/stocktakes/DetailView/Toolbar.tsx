    ...
    <div class="flex flex-col gap-3 py-3">
      <div class="flex flex-wrap items-end gap-4">
        <label class="flex flex-col gap-1 text-sm">
          <span class="text-xs font-medium text-gray-muted">Description</span>
          <input
            class="w-72 rounded-lg border border-line bg-page px-3 py-2 text-sm disabled:bg-row-hover disabled:text-gray-muted"
            value={description()}
        ...
        <label class="flex flex-1 flex-col gap-1 text-sm">
          <span class="text-xs font-medium text-gray-muted">Filter items</span>
          <input
            type="search"
            placeholder="Filter by item code or name…"
            class="w-full max-w-md rounded-lg border border-line bg-page px-3 py-2 text-sm"
      ...
      <Show when={props.disabled}>
        <div class="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800">
          {props.stocktake.isLocked
            ? "This stocktake is on hold and cannot be edited."
            : "This stocktake is finalised and cannot be edited."}
        </div>
      </Show>
    </div>
  );
